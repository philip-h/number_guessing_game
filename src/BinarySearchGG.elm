module BinarySearchGG exposing (..)
        ...
        [ toUnstyled <|
            ...
            div
                ...
                , div
                                ...
                                let
                                    elementGuessed =
                                        Array.get guess model.array
                                            |> Maybe.withDefault -1
                                in
                                case compare model.sampleElementIndex guess of
                                    -- Implement binary search logic
                                    GT ->
                                        [ text "The number in my head is"
                                        , span [ css [ Tw.text_color themeDark.accent ] ] [ text " bigger " ]
                                        , text ("than " ++ String.fromInt elementGuessed)
                                        ]

                                    LT ->
                                        [ text "The number in my head is"
                                        , span [ css [ Tw.text_color themeDark.accent ] ] [ text " smaller " ]
                                        , text ("than " ++ String.fromInt elementGuessed)
                                        ]
                    ...
                    , p [ css [ Tw.m_2 ] ] [ text "[ The numbers behind the boxes are sorted ]" ]
                    , viewArray model
                    , viewStatus model
                    , hr [ css [ Tw.m_4 ] ] []
                    , viewStats model
        ...
        |> Random.andThen
            (\randomList ->
                case randomList of
                    x :: xs ->
                        Random.uniform x xs
                            |> Random.map (\element -> ( List.sort randomList, element ))
...
init _ =
    ( { array = Array.empty
      ...
      , currentGuess = Nothing
      , guessCount = 0
      , maxGuessesNeeded = ceiling (logBase 2 <| toFloat arraySize) + 1
      , status = GameLoading
      , winCount = 0
      , loseCount = 0
      }
